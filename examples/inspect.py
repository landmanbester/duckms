...
print("\n3. VIEW DEFINITION:")
try:
    view_def = conn.execute("SELECT sql FROM duckdb_views() WHERE view_name = 'main_view';").fetchone()
    if view_def:
        print("   main_view SQL:")
        print("   ", view_def[0])
    else:
...
result = conn.execute("SELECT TIME, ANTENNA1, ANTENNA2, UVW, DATA_REAL, DATA_IMAG FROM main_view LIMIT 15;").fetchall()
for r in result:
    print('time = ', r[0])
    print('\n')
